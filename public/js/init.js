...
jQuery(document).ready(function(){
	...
	jQuery(window).load('body', function(){
		setTimeout(function(){
        jQuery('.tm_preloader').addClass('loaded');
    }, 1000);
	});
...
function tm_responsive(){
	...
	var leftpart			= jQuery('.tm_leftpart_wrap');
	var rightpart			= jQuery('.tm_rightpart');
...
function tm_portfolio(){
	...
	if(jQuery().isotope) {
		...
		// Needed variables
		var list 		 = jQuery('.tm_portfolio_list');
		var filter		 = jQuery('.tm_portfolio_filter');
...
function tm_owl_carousel(){
	...
	var carousel			= jQuery('.tm_services_wrap .owl-carousel');
	...
	jQuery('.tm_services_wrap .custom_nav > a.prev').on('click', function(){
		carousel.trigger('prev.owl.carousel');
		return false;
	});
	
	jQuery('.tm_services_wrap .custom_nav > a.next').on('click', function(){
		carousel.trigger('next.owl.carousel');
		return false;
	});
	tm_imgtosvg();
	
	var carusel2			= jQuery('.tm_testimonial_wrap .owl-carousel');
...
function tdProgress(container){
	...
	container.find('.tm_progress').each(function(i) {
		var progress 		= jQuery(this);
		var pValue 			= parseInt(progress.data('value'), 10);
		var pColor			= progress.data('color');
		var pBarWrap 		= progress.find('.tm_bar_wrap');
		var pBar 			= progress.find('.tm_bar');
		pBar.css({width:pValue+'%', backgroundColor:pColor});
		setTimeout(function(){pBarWrap.addClass('open');},(i*500));
	});
}
jQuery('.tm_progress_wrap').each(function() {
	"use strict";
...
// -----------------------------------------------------
 ...
 function tm_miniboxes(){
	 
  "use strict";
	 
  var el 			= jQuery('.tm_miniboxes');
	 
  if(el.length){
   el.each(function(index, element) {
         
    var child		= jQuery(element).find('.tm_minibox');
...
function tm_totop(){
	...
	jQuery(".tm_totop").on('click', function(e) {
		e.preventDefault();		
...
function tm_totop_myhide(){
	
	"use strict";
	
	var toTop		=jQuery(".tm_totop");
	if(toTop.length){
...
function tm_animate_text(){
	...
	var animateSpan			= jQuery('.tm_animation_text_word');
	
		animateSpan.typed({
			strings: ["Game Developer", "ML Enthusiast"],
			loop: true,
			startDelay: 1e3,
			backDelay: 2e3
		});
...
function tm_popup_blog(){
	"use strict";
	var li				= jQuery('.tm_list_wrap.blog_list .inner_list');
	var popupBox		= jQuery('#tm_popup_blog');
	var popupInner		= popupBox.find('.inner_popup');
	var closePopup		= popupBox.find('.close');
	
	li.each(function(){
		var element		= jQuery(this);
		var button		= element.find('');
...
function tm_popupscroll(){
	...
	var popupBox		= jQuery('.tm_popup_blog .inner_popup');
...
function tm_kenburn_slider(){
	
	"use strict";
	
		jQuery(function() {
			jQuery('.tm_hero_header_wrap .overlay_slider').vegas({
			timer:false,	
...
function tm_switcher(){
	
	"use strict";
	
	var switcherOpener				= jQuery('.tm_resize');
	var switcherIcon				= jQuery('.tm_leftpart_wrap .tm_resize i');
	var leftPart					= jQuery('.tm_leftpart_wrap');
	var rightPart					= jQuery('.tm_rightpart');
	...
	switcherOpener.on('click',function(){
		...
		if(jQuery('.jarallax').length){
			...
			setTimeout(function(){tm_jarallax();},300);
...
function tm_hamburger(){
	
	"use strict";
	
	var hamburger 		= jQuery('.hamburger');
	var mobileMenu		= jQuery('.tm_mobile_menu_wrap');
